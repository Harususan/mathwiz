...
newtonBackward = Blueprint("newtonBackward", __name__)


@newtonBackward.route("/run", methods=["POST"])
def post():
    ...
    try:
        ...
        ax.set_title(f"Newton Backward Interpolation")
        ax.set_xlabel("x")
        ax.set_ylabel("y")
...
@newtonBackward.route("/table", methods=["POST"])
def table():
    """
...
@newtonBackward.route("/runtable", methods=["POST"])
def post1():
    # Code to accept input from the user.
    input = request.json
    ...
    try:
        h = X[1] - X[0]
        # u=r'\left( \frac{'+sp.latex(x-X[0])+r'}{'+sp.latex(h)+r'} \right)'
        u = r"\frac{" + sp.latex(x - X[n - 1]) + r"}{" + sp.latex(h) + r"}"
        u1 = x - X[n - 1]
        u2 = r"\frac{" + sp.latex(x - X[n - 1]) + r"}{" + sp.latex(h) + r"}"
        ...
        for i in range(2, n):
            ...
            # display(Math(expr3)) for i in range(2, n):
...
@newtonBackward.route("/runner", methods=["POST"])
def post2():
    input = request.json
